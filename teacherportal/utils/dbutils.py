...
def deleteAssignment(assignment):
    ...
    -------
    bool
        If the object was found in the database for deletion or not.
    ...
    '''
    if not isinstance(assignment, Assignment):
        raise TypeError("Attempted to delete an object which is not an Assignment.")
    if loadAssignment(assignment.ID) == False:
        return False
    else:
        course = loadCourse(assignment.courseID)
        course.assignmentIDs.remove(assignment.ID)
        saveCourse(course)
        teacherCollection.delete_one({"ID": assignment.ID})
...
def deleteCourse(course):
    ...
    '''
    if not isinstance(course, Course):
        raise TypeError("Attempted to delete an object which is not a Course.")
    if loadCourse(course.ID) == False:
        return False
    else:
        for teacher in resolveIDList(course.teacherIDs, 'Teacher'):
            teacher.courseIDs.remove(course.ID)
            saveTeacher(teacher)
        for student in resolveIDList(course.studentIDs, 'Student'):
            student.courseIDs.remove(course.ID)
            saveStudent(student)
        for assignment in resolveIDList(course.assignmentIDs, 'Assignment'):
            deleteAssignment(assignment)
        courseCollection.delete_one({"ID": course.ID})
